...
func TestGetPrs_Run(t *testing.T) {
	...
	assert.Equal(t, 10, len(d.PullRequests))
}
...
func TestGetPrs_Run_ShowOnHold(t *testing.T) {
	...
	assert.Equal(t, len(d.PullRequests), 6)
}
...
func TestGetPrs_Run_ShowQueuedByDefault(t *testing.T) {
	d := NewGetPrs()
	...
	// by default the queued PR (#791) is included
	assert.Equal(t, 5, len(d.PullRequests))
	assert.Equal(t, 0, d.FilteredQueued)
}

func TestGetPrs_Run_HideQueued(t *testing.T) {
	d := NewGetPrs()
	d.HideQueued = true
	...
	// with --hide-queued the queued PR (#791) is filtered out
	assert.Equal(t, 4, len(d.PullRequests))
	assert.Equal(t, 1, d.FilteredQueued)
}
	...
	{"search":
		{"nodes":[
			...
			{"number":44,"title":"chore: prompt for version if not supplied","url":"https://{{.Host}}/plumming/dx/pull/44","createdAt":"2020-05-15T08:07:26Z","closed":false,"author":{"login":"me"},"repository":{"nameWithOwner":"plumming/dx"},"mergeable":"MERGEABLE","labels":{"nodes":[{"name":"size/XL"},{"name":"do-not-merge/hold"}]},"commits":{"nodes":[{"commit":{"status":{"contexts":[{"state":"PENDING","targetUrl":"","description":"Not mergeable. Needs approved label.","context":"Merge Status"},{"state":"SUCCESS","targetUrl":"https://dashboard/PR-44/4","description":"Pipeline successful","context":"pr-build"}]}}}]}},
     		{"number":791,"title":"chore: my-service to 0.0.717","url":"https://{{.Host}}/test_repo_two/pull/791","createdAt":"2020-05-14T23:12:56Z","closed":false,"isInMergeQueue":true,"author":{"login":"bot"},"repository":{"nameWithOwner":"my_owner"},"mergeable":"MERGEABLE","labels":{"nodes":[{"name":"size/XS"},{"name":"updatebot"}]},"commits":{"nodes":[{"commit":{"status":{"contexts":[{"state":"PENDING","targetUrl":"http://dec","description":"Not mergeable. Needs approved label.","context":"tide"},{"state":"SUCCESS","targetUrl":null,"description":"All Tasks have completed executing","context":"promotion-build"}]}}}]}},
